...
class ert_core:
  ...
  def __init__(self):
    ...
    #try:
    #  new_hostname = socket.gethostbyaddr('localhost')
  ...
  def build(self):
    if self.options.build:
      ...
      command = command_prefix + \
                ["-c","%s/Drivers/%s.cu" % (self.exe_path,self.dict["CONFIG"]["ERT_DRIVER"][0])] + \
      ...
      command = command_prefix + \
                ["-c","%s/Kernels/%s.cu" % (self.exe_path,self.dict["CONFIG"]["ERT_KERNEL"][0])] + \
